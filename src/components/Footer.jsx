...
const Footer = () => {
  return (
    <div  className='w-full h-screen bg-zinc-900 font-["Founders_Grotesk"] p-20 flex gap-5'>
      <div className="w-1/2 h-full flex flex-col justify-between">
        <div className="heading">
        <h1 className='text-[8vw] uppercase leading-none -mb-8 font-semibold '>eye-</h1>
        <h1 className='text-[8vw] uppercase leading-none -mb-8 font-semibold'>opening</h1>
        </div>
      ...
      <div className="w-1/2 ">
      <div className="heading">
        <h1 className='text-[5vw] uppercase leading-none -mb-10 font-semibold flex flex-row justify-between '>presentations-</h1>
        <div className="details font-['NeueMontreal'] mt-20 flex flex-col  ">
        <div className='mb-10'>
        <span className='text-2xl '>Social Medias:</span>
          {["facebook","Instagram","LinkedIn","WhatsApp"].map((item,id)=>(
            <a href="#" className='block text-xl font-light underline text-white ' key={id}>{item}</a>
          ))}
        </div>
          <div className='mb-10'>
          <span className='text-2xl '>Location:</span>
          {["jagatpur","Cuttack","754021","Odisha,India"].map((item,id)=>(
            <a href="#" className='block text-xl font-light underline text-white ' key={id}>{item}</a>
          ))}
          </div>
          <div className='mb-10'>
          <span className='text-2xl '>Email:</span>
          {["subhankar2118@gmail.com"].map((item,id)=>(
            <a href="#" className='block text-xl font-light underline text-white ' key={id}>{item}</a>
          ))}
          </div>
        </div>
        ...
        <span className='text-2xl '>Pages:</span>
          {["Home","About Us","Our Services","Join Us"].map((item,id)=>(
            <a href="#" className='block text-xl font-light underline text-white ' key={id}>{item}</a>
          ))}
        </div>
